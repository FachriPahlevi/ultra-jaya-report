...
function RichText({ text, maxLines = 3, stopPropagation = true }) {
    ...
    if (isList) {
        return (
            <div>
                ...
                {hasMore && (
                    <button onClick={toggle} className="flex items-center gap-1 text-[11px] text-blue-600 font-semibold mt-1.5 active:opacity-70">
                        {expanded ? <><ChevronUp className="w-3 h-3" /> Tampilkan lebih sedikit</> : <><ChevronDown className="w-3 h-3" /> +{lines.length - maxLines} lainnya</>}
                    </button>
                )}
            </div>
        );
    ...
    return (
        <div>
            ...
            {isLong && (
                <button onClick={toggle} className="flex items-center gap-1 text-[11px] text-blue-600 font-semibold mt-1 active:opacity-70">
                    {expanded ? <><ChevronUp className="w-3 h-3" /> Lebih sedikit</> : <><ChevronDown className="w-3 h-3" /> Selengkapnya</>}
                </button>
            )}
        </div>
    );
