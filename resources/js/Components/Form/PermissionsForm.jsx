        ...
        <ModalOverlay isOpen={isOpen} onClose={onClose}>
            <div className="bg-card rounded-2xl border border-border shadow-xl w-full max-w-[700px] max-h-[85vh] overflow-hidden">
                <div className="sticky top-0 bg-card border-b border-border px-6 py-4 flex items-center justify-between">
                    <h2 className="text-xl font-bold text-foreground">Permissions for {role.name}</h2>
                        ...
                        return (
                            <div key={group} className="mb-6 pb-2 border-b border-border/50">
                                <div className="flex items-center gap-3 mb-3">
                                    <button
                                        ...
                                        className={`w-4 h-4 rounded border-2 flex items-center justify-center ${
                                            ...
                                            isPartiallySelected ? "bg-primary/20 border-primary" : "border-border hover:border-primary"
                                        }`}
                                    >
                                        {isFullySelected && (
                                    ...
                                    <h3 className="text-[14px] font-bold text-foreground uppercase tracking-wide">{group}</h3>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-7">
                                    {perms.map((perm) => (
                                        <label key={perm.id} className="flex items-center gap-2 cursor-pointer group hover:bg-muted/50 p-1 rounded">
                                            <input
                                                type="checkbox"
                ...
                <div className="sticky bottom-0 bg-card border-t border-border px-6 py-4 flex gap-3">
                    ...
                    <BtnDefault onClick={handleSave} loading={processing} className="flex-[2]">Save Permissions</BtnDefault>
                </div>
            </div>
        </ModalOverlay>
    );
