    ...
    const confirmDeleteUser = (user) => {
        if (user.roles[0]?.name === "SUPER_ADMIN") {
            showStatusModal(
                "error",
                "Error",
                "User dengan role SUPER_ADMIN tidak dapat dihapus untuk menjaga keamanan sistem.",
            );
            return;
        }
        ...
        setStatusModalProps({
            ...
            button1: {
                ...
                onClick: async () => {
                    try {
                        // PERBAIKI URL INI
                        await axios.delete(`/settings/users/${user.id}`); // <-- HARUS INI
    ...
    const confirmDeleteRole = (role) => {
        if (role.name === "SUPER_ADMIN") {
            showStatusModal(
                "error",
                "Error",
                "Role SUPER_ADMIN adalah role sistem yang tidak dapat dihapus.",
            );
            return;
        }
            ...
            button1: {
                ...
                onClick: async () => {
                    try {
                        ...
                        router.reload(); // Ganti window.location.reload()
                    } catch (error) {
                        const message =
                            error.response?.data?.message ||
