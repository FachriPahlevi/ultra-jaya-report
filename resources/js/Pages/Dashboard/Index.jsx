...
import { HiOutlineDocumentReport, HiOutlineClock, HiOutlineCheckCircle, HiOutlineChartBar, HiOutlineUser, HiOutlineExternalLink, HiArrowUp, HiOutlinePlusCircle, HiArrowRight } from "react-icons/hi";
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className="inline-block px-2.5 py-0.5 rounded-full text-[11.5px] font-semibold whitespace-nowrap" style={{ background: cfg.bg, color: cfg.color }}>
      {cfg.label}
    </span>
  );
};
...
const SparkLine = ({ color = "var(--primary)" }) => (
  <svg viewBox="0 0 80 32" width="80" height="32" fill="none">
    <polyline points="0,28 13,22 26,24 39,14 52,16 65,8 80,4" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
...
export default function Dashboard({ stats = { total: 0, pending: 0, solved: 0, myReports: 0 }, recentReports = [], topArea = null }) {
    ...
    <AppLayout title="Dashboard">
      <div className="flex flex-col gap-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-[22px] font-bold text-[var(--foreground)] tracking-[-0.5px]">Dashboard</h1>
            <p className="text-[13.5px] text-[var(--muted-foreground)] mt-0.5">Welcome back, {firstName} 👋</p>
          </div>
          ...
          <div className="bg-[var(--card)] rounded-2xl border border-[var(--border)] p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-[38px] font-bold text-[var(--foreground)] tracking-[-1.5px] leading-none mt-1">{stats.total}</p>
                <p className="text-[12px] text-[var(--muted-foreground)] mt-1.5">All issues submitted across areas</p>
              </div>
          ...
          <div className="bg-[var(--card)] rounded-2xl border border-[var(--border)] p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-[38px] font-bold text-[var(--foreground)] tracking-[-1.5px] leading-none mt-1">{stats.pending}</p>
                <p className="text-[12px] text-[var(--muted-foreground)] mt-1.5">Issues awaiting action</p>
              </div>
          ...
          <div className="bg-[var(--card)] rounded-2xl border border-[var(--border)] p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <p className="text-[12px] text-[var(--muted-foreground)] mt-1">Issues resolved successfully</p>
              </div>
          ...
          <div className="bg-[var(--card)] rounded-2xl border border-[var(--border)] overflow-hidden">
            ...
            <div className="divide-y divide-[var(--border)]">
              {recentReports.length === 0 ? (
                <div className="text-center py-12 text-[var(--muted-foreground)] text-[13px]">No reports yet</div>
              ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-0.5">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 min-w-0">
                            <span className="text-[13px] font-semibold text-[var(--foreground)] truncate">{report.issue ?? "-"}</span>
                            <span className="text-[11px] text-[var(--muted-foreground)] truncate">by {report.submitted_by ?? "-"}</span>
                          </div>
