...
export default function MasterActivity({ activities = { data: [], links: [], meta: {} } }) {
    ...
    const submit = (e) => {
        e.preventDefault()
        if (editTarget) {
            put(`/master-activity/${editTarget.id}`, { onSuccess: closeModal })
        } else {
            post("/master-activity", { onSuccess: closeModal })
        }
    }

    const deleteActivity = (activity) => {
        if (!window.confirm(`Delete activity "${activity.name}"?`)) return
        router.delete(`/master-activity/${activity.id}`)
    }
        ...
        <AppLayout title="Master Activity">
            ...
            <div className="flex flex-col gap-5">
                <div className="flex items-center justify-between flex-wrap gap-3">
                    <div>
                        <h2 className="text-2xl font-bold text-foreground tracking-[-0.5px] m-0">Master Activity</h2>
                    </div>
                                ...
                                activities.data.map((activity, i) => (
                                    <tr key={activity.id} className="hover:bg-muted/50 transition-colors">
                                        <td className="p-2.5 text-[13px] text-muted-foreground font-semibold">
                                            {(activities.meta?.from ?? 0) + i}
                                        </td>
                                        <td className="p-2.5 text-[13px] font-semibold text-foreground">{activity.name}</td>
                                        <td className="p-2.5 text-[13px] text-foreground">{activity.description ?? "-"}</td>
                                        <td className="p-2.5">
                        ...
                        <form onSubmit={submit} className="flex flex-col gap-4">
                            ...
                            <div className="flex gap-2.5 mt-2">
                                ...
                                <BtnDefault
                                    ...
                                    className="flex-[2]"
                                >
                                    {processing ? "Saving..." : editTarget ? "Save Changes" : "Add Activity"}
                                </BtnDefault>
                            </div>
                        </form>
                    </div>
